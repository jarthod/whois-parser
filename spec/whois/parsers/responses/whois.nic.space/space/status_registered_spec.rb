...
describe "whois.nic.space", :aggregate_failures do

  subject do
    file = fixture("responses", "whois.nic.space/space/status_registered.txt")
    part = Whois::Record::Part.new(body: File.read(file), host: "whois.nic.space")
  ...
  it "matches status_registered.expected" do
    ...
    expect(subject.created_on).to eq(DateTime.parse("2014-04-10 09:14:07 UTC"))
    expect(subject.updated_on).to be_a(Time)
    expect(subject.updated_on).to eq(DateTime.parse("2021-06-01 11:38:04 UTC"))
    expect(subject.expires_on).to be_a(Time)
    expect(subject.expires_on).to eq(DateTime.parse("2022-04-10 23:59:59 UTC"))
    expect(subject.registrar).to be_a(Whois::Parser::Registrar)
    expect(subject.registrar.id).to eq("9999")
    expect(subject.registrar.name).to eq("CentralNic Ltd")
    expect(subject.registrar.url).to eq("https://www.centralnic.com/")
    ...
    expect(subject.nameservers[0].name).to eq("a.nic.space")
    expect(subject.nameservers[1]).to be_a(Whois::Parser::Nameserver)
    expect(subject.nameservers[1].name).to eq("b.nic.space")
    expect(subject.nameservers[2]).to be_a(Whois::Parser::Nameserver)
    expect(subject.nameservers[2].name).to eq("e.nic.space")
    expect(subject.nameservers[3]).to be_a(Whois::Parser::Nameserver)
    expect(subject.nameservers[3].name).to eq("f.nic.space")
  end
end
