...
describe "whois.nic.bike", :aggregate_failures do
  ...
  it "matches status_registered.expected" do
    expect(subject.domain).to eq("bike.bike")
    expect(subject.domain_id).to eq("a864ce4015614eeda646a4121b91c784-DONUTS")
    expect(subject.status).to eq(:registered)
    expect(subject.available?).to eq(false)
    expect(subject.registered?).to eq(true)
    expect(subject.created_on).to be_a(Time)
    expect(subject.created_on).to eq(DateTime.parse("2015-07-19 17:01:03 UTC"))
    expect(subject.updated_on).to be_a(Time)
    expect(subject.updated_on).to eq(DateTime.parse("2022-09-02 17:01:11 UTC"))
    expect(subject.expires_on).to be_a(Time)
    expect(subject.expires_on).to eq(DateTime.parse("2023-07-19 17:01:03 UTC"))
    expect(subject.registrar).to be_a(Whois::Parser::Registrar)
    expect(subject.registrar.id).to eq("1011")
    expect(subject.registrar.name).to eq("101domain GRS Limited")
    expect(subject.registrar.url).to eq("http://101domain.com")
    ...
    expect(subject.registrant_contacts[0].organization).to eq("Digital Privacy Corporation")
    ...
    expect(subject.nameservers[0].name).to eq("ns1.gagarin2501.hospedagemdesites.ws")
    expect(subject.nameservers[1]).to be_a(Whois::Parser::Nameserver)
    expect(subject.nameservers[1].name).to eq("ns2.gagarin2501.hospedagemdesites.ws")
  end
end
