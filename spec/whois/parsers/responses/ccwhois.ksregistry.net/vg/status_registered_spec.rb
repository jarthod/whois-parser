...
describe "ccwhois.ksregistry.net", :aggregate_failures do

  subject do
    file = fixture("responses", "ccwhois.ksregistry.net/vg/status_registered.txt")
    part = Whois::Record::Part.new(body: File.read(file), host: "ccwhois.ksregistry.net")
  ...
  it "matches status_registered.expected" do
    ...
    expect(subject.domain).to eq("google.vg")
    expect { subject.domain_id }.to raise_error(Whois::AttributeNotSupported)
    expect(subject.status).to eq(:registered)
    expect(subject.available?).to eq(false)
    expect(subject.registered?).to eq(true)
    expect(subject.created_on).to be_a(Time)
    expect(subject.created_on).to eq(DateTime.parse("1999-06-05 00:00:00"))
    expect(subject.updated_on).to be_a(Time)
    expect(subject.updated_on).to eq(DateTime.parse("2013-03-01 00:02:14"))
    expect(subject.expires_on).to be_a(Time)
    expect(subject.expires_on).to eq(DateTime.parse("2013-06-05 00:00:00"))
