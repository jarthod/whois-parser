...
describe "whois.nic.fm", :aggregate_failures do

  subject do
    file = fixture("responses", "whois.nic.fm/fm/status_registered.txt")
    part = Whois::Record::Part.new(body: File.read(file), host: "whois.nic.fm")
    Whois::Parser.parser_for(part)
  end

  it "matches status_registered.expected" do
    expect(subject.domain).to eq("google.fm")
    expect(subject.domain_id).to eq("D34865469-CNIC")
    expect(subject.status).to eq(:registered)
    expect(subject.available?).to eq(false)
    expect(subject.registered?).to eq(true)
    expect(subject.created_on).to be_a(Time)
    expect(subject.created_on).to eq(DateTime.parse("2000-09-05T23:59:59.0Z"))
    expect(subject.updated_on).to be_a(Time)
    expect(subject.updated_on).to eq(DateTime.parse("2017-10-20T17:48:39.0Z"))
    expect(subject.expires_on).to be_a(Time)
    expect(subject.expires_on).to eq(DateTime.parse("2018-09-04T23:59:59.0Z"))
    ...
    expect(subject.registrant_contacts[0].id).to eq("C78398194-CNIC")
    ...
    expect(subject.admin_contacts[0].id).to eq("C78398194-CNIC")
    ...
    expect(subject.technical_contacts[0].id).to eq("C78398194-CNIC")
