...
describe "whois.verisign-grs.com", :aggregate_failures do
  ...
  subject do
    file = fixture("responses", "whois.verisign-grs.com/net/status_registered.txt")
  ...
  it "matches status_registered.expected" do
    ...
    expect(subject.domain).to eq("google.net")
    expect(subject.domain_id).to eq("4802712_DOMAIN_NET-VRSN")
    expect(subject.status).to eq(:registered)
    expect(subject.available?).to eq(false)
    expect(subject.registered?).to eq(true)
    expect(subject.created_on).to be_a(Time)
    expect(subject.created_on).to eq(DateTime.parse("1999-03-15 05:00:00 UTC"))
    expect(subject.updated_on).to be_a(Time)
    expect(subject.updated_on).to eq(DateTime.parse("2017-02-11 10:56:37 UTC"))
    expect(subject.expires_on).to be_a(Time)
    expect(subject.expires_on).to eq(DateTime.parse("2018-03-15 04:00:00 UTC"))
