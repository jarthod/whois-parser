...
#
#   /spec/fixtures/responses/whois.nic.google/app/status_registered.expected
#
...
describe "whois.nic.google", :aggregate_failures do
  ...
  subject do
    file = fixture("responses", "whois.nic.google/app/status_registered.txt")
    part = Whois::Record::Part.new(body: File.read(file), host: "whois.nic.google")
    Whois::Parser.parser_for(part)
  end

  it "matches status_registered.expected" do
    expect(subject.domain).to eq("google.app")
    expect(subject.domain_id).to eq("2C5A4BAFF-APP")
    expect(subject.status).to eq(:registered)
    expect(subject.available?).to eq(false)
    expect(subject.registered?).to eq(true)
    expect(subject.created_on).to be_a(Time)
    expect(subject.created_on).to eq(DateTime.parse("2018-03-29T16:02:13Z"))
    expect(subject.updated_on).to be_a(Time)
    expect(subject.updated_on).to eq(DateTime.parse("2019-03-03T10:46:31Z"))
    expect(subject.expires_on).to be_a(Time)
    expect(subject.expires_on).to eq(DateTime.parse("2020-03-29T16:02:13Z"))
