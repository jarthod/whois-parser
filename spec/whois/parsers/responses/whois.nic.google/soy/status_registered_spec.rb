...
#
#   /spec/fixtures/responses/whois.nic.google/soy/status_registered.expected
#
...
describe "whois.nic.google", :aggregate_failures do
  ...
  subject do
    file = fixture("responses", "whois.nic.google/soy/status_registered.txt")
    part = Whois::Record::Part.new(body: File.read(file), host: "whois.nic.google")
    Whois::Parser.parser_for(part)
  end

  it "matches status_registered.expected" do
    expect(subject.domain).to eq("google.soy")
    expect(subject.domain_id).to eq("88628D1-SOY")
    expect(subject.status).to eq(:registered)
    expect(subject.available?).to eq(false)
    expect(subject.registered?).to eq(true)
    expect(subject.created_on).to be_a(Time)
    expect(subject.created_on).to eq(DateTime.parse("2014-12-01T21:58:34Z"))
    expect(subject.updated_on).to be_a(Time)
    expect(subject.updated_on).to eq(DateTime.parse("2019-02-14T19:53:13Z"))
    expect(subject.expires_on).to be_a(Time)
    expect(subject.expires_on).to eq(DateTime.parse("2019-12-01T21:58:34Z"))
