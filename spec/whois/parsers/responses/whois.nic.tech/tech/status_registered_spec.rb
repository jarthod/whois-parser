...
describe "whois.nic.tech", :aggregate_failures do

  subject do
    file = fixture("responses", "whois.nic.tech/tech/status_registered.txt")
    part = Whois::Record::Part.new(body: File.read(file), host: "whois.nic.tech")
  ...
  it "matches status_registered.expected" do
    ...
    expect(subject.domain).to eq("google.tech")
    expect(subject.domain_id).to eq("D9157622-CNIC")
    expect(subject.status).to eq(:registered)
    expect(subject.available?).to eq(false)
    expect(subject.registered?).to eq(true)
    expect(subject.created_on).to be_a(Time)
    expect(subject.created_on).to eq(DateTime.parse("2015-07-29 14:20:05 UTC"))
    expect(subject.updated_on).to be_a(Time)
    expect(subject.updated_on).to eq(DateTime.parse("2021-08-25 01:16:43 UTC"))
    expect(subject.expires_on).to be_a(Time)
    expect(subject.expires_on).to eq(DateTime.parse("2022-07-29 23:59:59 UTC"))
